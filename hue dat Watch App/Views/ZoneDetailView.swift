...
struct ZoneDetailView: View {
    ...
    private func turnOff() async {
        ...
        guard let zone = zone,
              let groupedLight = zone.groupedLights?.first else {
            print("❌ turnOff blocked: zone=\(zone != nil), groupedLights=\(zone?.groupedLights != nil), count=\(zone?.groupedLights?.count ?? 0)")
            return
        }
        ...
        do {
            ...
            // Update local state
            bridgeManager.updateLocalZoneState(zoneId: zoneId, on: false)
